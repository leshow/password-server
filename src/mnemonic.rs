...
impl Mnemonic {
    pub fn new(chars: &str) -> Mnemonic {
        let h = Mnemonic::gen_sha256(chars);
        println!("{:?}", h);
        let length = chars.len() / 32;
        println!("{:?}", &h.as_ref()[..length]);
        Mnemonic {
            mnemonic: [chars.as_ref(), &h.as_ref()[..length]].concat(),
        }
    }
    ...
    pub fn to_words<'a>(&'a self, wordslist: &'a [String]) -> Vec<&str> {
        ...
        if let Ok((_, bit_sequence)) = bit_vec(self.mnemonic.as_slice()) {
    ...
    fn gen_sha256<S: AsRef<str>>(s: S) -> Digest {
        digest::digest(DIGEST_ALG, s.as_ref().as_bytes())
    }
}
